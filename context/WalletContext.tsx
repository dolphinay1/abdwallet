...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  const resetInactivityTimer = useCallback(() => {
    if (inactivityTimer.current) clearTimeout(inactivityTimer.current);
    inactivityTimer.current = setTimeout(() => wipeABDWallet(), 5 * 60 * 1000);
  }, [wipeABDWallet]);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      try { sessionStorage.removeItem('_abd_tab_lock'); } catch {}
    };
  }, []);

  // Wipe on unload — keep session so refresh can restore
  useEffect(() => {
    const handler = () => wipeABDWallet({ keepSession: true });
  ...
  useEffect(() => {
    ...
    trap.addEventListener('input', () => wipeABDWallet());
    trap.addEventListener('change', () => wipeABDWallet());
