...
export async function generateBiometricKey(userLabel = 'abd-vault'): Promise<string> {
  ...
  try {
    ...
    const credential = await navigator.credentials.create({
      publicKey: {
        challenge,
        rp: { name: 'ABD Wallet', id: window.location.hostname },
        user: {
          id: userId,
          name: userLabel,
          displayName: 'ABD Vault',
        },
        pubKeyCredParams: [
...
async function deriveFallbackKey(): Promise<string> {
  ...
  const key = await crypto.subtle.deriveKey(
    {
      name: 'PBKDF2',
      salt: new TextEncoder().encode('abd-fallback'),
