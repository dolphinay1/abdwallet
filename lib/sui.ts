...
async function rpc<T>(method: string, params: unknown[]): Promise<T> {
  ...
  // Route reads through /api/proxy — the public SUI fullnode does not allow
  // CORS from browser origins (e.g. localhost). Fall back to a direct call.
  try {
    const proxyRes = await fetch('/api/proxy', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ url: RPC, method: 'POST', payload }),
    });
    if (proxyRes.ok) {
      const proxied = await proxyRes.json() as { result?: T; error?: { message: string } };
      if (proxied.error) throw new Error(proxied.error.message);
      if (proxied.result !== undefined) return proxied.result as T;
    }
  } catch (err) {
    // Proxy JSON-RPC error — surface it; transport errors fall through to direct
    if (err instanceof Error && !/fetch|proxy|HTTP/i.test(err.message)) throw err;
  }
